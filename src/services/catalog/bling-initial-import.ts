...
export async function runInitialImportStep(): Promise<InitialImportStepResult> {
  ...
  console.log(`[bling/initial-import] processando página ${page}`);
  ...
  try {
    ...
    console.log(
      `[bling/initial-import] página ${page} ok — mapped=${products.length} next=${nextPage}`,
    );
    ...
    console.error("[bling/initial-import] erro:", message);
...
export async function runInitialImportIfNeeded(): Promise<InitialImportStepResult> {
  ...
  if (!shouldRunInitialImport(importDone, catalogCount)) {
    console.log(
      `[bling/initial-import] ignorado (importDone=${importDone}, catalogCount=${catalogCount})`,
    );
    return { status: "skipped" };
  }
